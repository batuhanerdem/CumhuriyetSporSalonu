...
class NotVerifiedFragment : Fragment() {
    ...
    private fun decideString() {
        if (args.verifiedStatus == VerifiedStatus.NOTANSWERED) {
            setString(
                R.string.wait_for_verify_title.stringfy(),R.string.wait_for_verify_info.stringfy(),R.string.wait_for_verify.stringfy()
            )
            setPhoto(R.drawable.photo_waiting)

        } else {
            setString(R.string.verify_denied_title.stringfy(),R.string.verify_denied_info.stringfy(), R.string.verify_denied.stringfy())
